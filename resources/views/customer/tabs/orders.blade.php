<div>
    <h2 class="text-xl font-bold text-gray-900 mb-6">{{ __('portal.my_orders') }}</h2>

    {{-- Filters --}}
    <div class="flex gap-2 mb-6 overflow-x-auto pb-2">
        @foreach(['todos' => __('All'), 'activos' => __('Active'), 'entregados' => __('portal.delivered'), 'cancelados' => __('portal.cancelled')] as $key => $label)
    ...
    @if($orders->isEmpty())
        <div class="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <p class="text-gray-500 mb-4">{{ $currentFilter !== 'todos' ? __('portal.no_orders_filter') : __('portal.no_orders') }}</p>
            ...
            @foreach($orders as $order)
                <a href="{{ route('customer.order.detail', $order) }}" class="block bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow p-5">
                    <div class="flex items-start justify-between mb-3">
                        <div>
                            <span class="text-sm font-semibold text-gray-900">{{ __('portal.order') }} #{{ $order->id }}</span>
                    ...
                    <div class="text-sm text-gray-600 mb-3">
                        ...
                        @if($order->lines->count() > 3)
                            <span class="text-gray-400">+{{ $order->lines->count() - 3 }} {{ __('portal.more') }}</span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        @if($order->balance > 0)
                            <span class="text-amber-600 font-medium">{{ __('Balance') }}: ${{ number_format($order->balance, 2) }}</span>
                        @else
