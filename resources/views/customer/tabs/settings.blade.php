<div>
    <h2 class="text-xl font-bold text-gray-900 mb-6">{{ __('portal.settings') }}</h2>

    {{-- Notifications --}}
    <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
        <h3 class="text-sm font-semibold text-gray-900 mb-4">{{ __('Notifications') }}</h3>
        <form method="POST" action="{{ route('customer.notifications.update') }}">
            ...
            <div class="space-y-4">
                <label class="flex items-center justify-between cursor-pointer">
                    <div>
                        <p class="text-sm font-medium text-gray-900">{{ __('portal.payment_confirmations') }}</p>
                        <p class="text-xs text-gray-500">{{ __('portal.payment_confirmations_desc') }}</p>
                    </div>
                ...
                <label class="flex items-center justify-between cursor-pointer">
                    <div>
                        <p class="text-sm font-medium text-gray-900">{{ __('portal.promotions_updates') }}</p>
                        <p class="text-xs text-gray-500">{{ __('portal.promotions_updates_desc') }}</p>
                    </div>
    ...
    {{-- Security --}}
    <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
        <h3 class="text-sm font-semibold text-gray-900 mb-4">{{ __('portal.security') }}</h3>
        <div class="space-y-3">
            <a href="{{ route('customer.profile') }}" class="flex items-center justify-between py-2 text-sm text-gray-600 hover:text-primary-600 transition-colors">
                <span>{{ __('portal.change_password') }}</span>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-red-100" x-data="{ confirmDelete: false }">
        <h3 class="text-sm font-semibold text-red-600 mb-4">{{ __('portal.danger_zone') }}</h3>

        <div class="space-y-3">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-900">{{ __('portal.delete_account') }}</p>
                    <p class="text-xs text-gray-500">{{ __('portal.delete_account_desc') }}</p>
                </div>
        ...
        <div x-show="confirmDelete" x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" @keydown.escape.window="confirmDelete = false">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6" @click.away="confirmDelete = false">
                <h3 class="text-lg font-bold text-gray-900 mb-2">{{ __('portal.delete_account_confirm') }}</h3>
                <p class="text-sm text-gray-600 mb-4">{{ __('portal.delete_account_confirm_desc') }}</p>
                ...
                <form method="POST" action="{{ route('customer.account.delete') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="delete_password" class="block text-sm font-medium text-gray-700 mb-1">{{ __('Password') }}</label>
