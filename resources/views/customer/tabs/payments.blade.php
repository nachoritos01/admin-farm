<div>
    <h2 class="text-xl font-bold text-gray-900 mb-6">{{ __('portal.payments') }}</h2>

    {{-- Summary cards --}}
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl shadow-sm p-5">
            <p class="text-sm text-gray-500 mb-1">{{ __('portal.total_paid') }}</p>
            <p class="text-2xl font-bold text-green-600">${{ number_format($totalPaid, 2) }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm p-5">
            <p class="text-sm text-gray-500 mb-1">{{ __('portal.pending_balance') }}</p>
            <p class="text-2xl font-bold {{ $pendingBalance > 0 ? 'text-amber-600' : 'text-gray-400' }}">${{ number_format($pendingBalance, 2) }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm p-5">
            <p class="text-sm text-gray-500 mb-1">{{ __('portal.active_orders') }}</p>
    ...
    @if($ordersWithBalance->isNotEmpty())
        <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
            <h3 class="text-sm font-semibold text-gray-900 mb-4">{{ __('portal.orders_with_balance') }}</h3>
            <div class="space-y-3">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <h3 class="text-sm font-semibold text-gray-900 mb-4">{{ __('portal.payment_history') }}</h3>

        @if($allPayments->isEmpty())
            <p class="text-sm text-gray-500 text-center py-8">{{ __('portal.no_payments') }}</p>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100">
                            ...
                            <th class="text-left px-6 py-2 text-xs font-semibold text-gray-500 uppercase">{{ __('Order') }}</th>
                            <th class="text-left px-6 py-2 text-xs font-semibold text-gray-500 uppercase">{{ __('portal.method') }}</th>
                            <th class="text-right px-6 py-2 text-xs font-semibold text-gray-500 uppercase">{{ __('Amount') }}</th>
                        </tr>
                    </thead>
    ...
    <div class="mt-6 bg-blue-50 border border-blue-100 rounded-2xl p-5">
        <h4 class="text-sm font-semibold text-blue-900 mb-2">{{ __('portal.accepted_payment_methods') }}</h4>
        <div class="flex flex-wrap gap-3 text-sm text-blue-700">
            <span class="flex items-center gap-1">{{ __('portal.cash') }}</span>
            <span class="text-blue-300">|</span>
            <span class="flex items-center gap-1">{{ __('portal.bank_transfer') }}</span>
            <span class="text-blue-300">|</span>
            <span class="flex items-center gap-1">{{ __('portal.credit_debit_card') }}</span>
        </div>
    </div>
</div>
