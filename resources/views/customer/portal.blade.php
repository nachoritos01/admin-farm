<x-layouts.app :title="__('portal.my_account')">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <aside class="hidden lg:block lg:w-72 shrink-0">
                <div class="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
                    ...
                    <nav class="space-y-1">
                        ...
                        <a href="{{ route('customer.loyalty') }}"
                           class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors {{ $tab === 'loyalty' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50' }}">
                            ...
                            {{ __('portal.loyalty') }}
                            <span class="text-[10px] font-bold text-orange-500 bg-orange-50 px-1.5 py-0.5 rounded-full ml-auto">{{ __('portal.new') }}</span>
                        </a>
                        @endif
                    ...
                    <div class="mt-4 px-4 space-y-1">
                        <a href="{{ route('terms') }}" target="_blank" class="block text-xs text-gray-400 hover:text-gray-600 transition-colors">{{ __('portal.terms_conditions') }}</a>
                        <a href="{{ route('usage-policies') }}" target="_blank" class="block text-xs text-gray-400 hover:text-gray-600 transition-colors">{{ __('portal.usage_policies') }}</a>
                    </div>
                </div>
            </aside>
            ...
            <div class="lg:hidden mb-6 -mx-4 px-4 overflow-x-auto">
                <div class="flex gap-2 min-w-max pb-2">
                    ...
                    <a href="{{ route('customer.loyalty') }}" class="px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors {{ $tab === 'loyalty' ? 'bg-primary-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100' }}">
                        {{ __('portal.loyalty') }} <span class="text-[10px] font-bold text-orange-500 bg-orange-50 px-1 py-0.5 rounded-full">{{ __('portal.new') }}</span>
                    </a>
                    @endif
        ...
        <div class="lg:hidden mt-8 text-center">
            ...
            <div class="mt-3 flex justify-center gap-4">
                <a href="{{ route('terms') }}" target="_blank" class="text-xs text-gray-400 hover:text-gray-600 transition-colors">{{ __('portal.terms_conditions') }}</a>
                <a href="{{ route('usage-policies') }}" target="_blank" class="text-xs text-gray-400 hover:text-gray-600 transition-colors">{{ __('portal.usage_policies') }}</a>
            </div>
        </div>
    </div>
</x-layouts.app>
