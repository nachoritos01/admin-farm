<x-layouts.saas title="Pricing — SaaS Template">
    <section class="py-20 bg-gradient-to-br from-blue-50 to-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h1 class="text-4xl sm:text-5xl font-extrabold text-gray-900">{{ __('Simple, transparent pricing') }}</h1>
                <p class="mt-4 text-lg text-gray-600">{{ __('Choose the plan that fits your business. Change or cancel anytime.') }}</p>
            </div>
                ...
                <div class="flex justify-center mb-12">
                    <div class="bg-gray-100 rounded-full p-1 flex items-center gap-1">
                        ...
                        <button @click="yearly = true" :class="yearly ? 'bg-white shadow text-gray-900' : 'text-gray-500'" class="px-4 py-2 rounded-full text-sm font-medium transition">
                            {{ __('Annual') }} <span class="text-green-600 font-semibold">-20%</span>
                        </button>
                    </div>
                </div>
    ...
    <section class="py-20 bg-white">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-3xl font-bold text-gray-900 text-center mb-12">{{ __('Frequently asked questions') }}</h2>

            <div class="space-y-4" x-data="{ open: null }">
                @php
                    $faqs = [
                        ['q' => __('Do I need a credit card to get started?'), 'a' => __('No. The 14-day free trial does not require a credit card. Just enter your email and you\'re good to go.')],
                        ['q' => __('Can I change my plan later?'), 'a' => __('Yes, you can upgrade or downgrade your plan at any time. The change takes effect immediately and billing is prorated.')],
                        ['q' => __('What happens when my trial ends?'), 'a' => __('We\'ll notify you 3 days before. If you don\'t choose a plan, your account is paused but your data remains safe. You can reactivate anytime.')],
                        ['q' => __('Do prices include taxes?'), 'a' => __('Prices shown do not include taxes. Applicable taxes are added at checkout and reflected on your invoice.')],
                        ['q' => __('Can I cancel anytime?'), 'a' => __('Yes, you can cancel your subscription at any time with no penalty. Your access continues until the end of the paid period.')],
                        ['q' => __('Is my data secure?'), 'a' => __('Your data is completely isolated from other businesses. We use encryption in transit and automatic daily backups.')],
                    ];
                @endphp
    ...
    <section class="py-16 bg-blue-600">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 class="text-3xl font-bold text-white mb-4">{{ __('Ready to organize your business?') }}</h2>
            <p class="text-lg text-blue-100 mb-8">{{ __('14 days free. No credit card required.') }}</p>
