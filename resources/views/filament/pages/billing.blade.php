...
<x-filament-panels::page>
    <div class="space-y-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <p class="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                        @if($cancellation['was_charged'])
                            {{ __('Your :plan plan will be cancelled on :date. No additional charges will be made.', ['plan' => $cancellation['plan'], 'date' => $cancellation['ends_at']->format('F j, Y')]) }}
                        @else
                            {{ __('Your trial period ends on :date. No charges were made.', ['date' => $cancellation['ends_at']->format('F j, Y')]) }}
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ __(':plan Plan', ['plan' => $tenant?->planLabel()]) }}
                    </h3>
                    <p class="text-sm text-gray-500 dark:text-white mt-1">
                        @if($cancellation && $cancellation['was_charged'])
                            {{ __('Active until :date', ['date' => $cancellation['ends_at']->format('F j, Y')]) }}
                        @elseif($cancellation)
                            {{ __('Trial until :date', ['date' => $cancellation['ends_at']->format('F j, Y')]) }}
                        @elseif($isSubscribed)
                            {{ __('Active subscription') }}
                            @if($nextBillingDate)
                                — {{ __('next charge on :date', ['date' => $nextBillingDate]) }}
                            @endif
                        @elseif($isOnTrial)
                            {{ __('Trial period') }} — {{ __(':days days remaining', ['days' => $tenant->trialDaysRemaining()]) }}
                            @if($nextBillingDate)
                                — {{ __('first charge on :date', ['date' => $nextBillingDate]) }}
                            @endif
                        @else
        ...
        {{-- Usage Stats --}}
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Current Usage') }}</h3>
            <div class="space-y-4">
        ...
        @if (!empty($pluginAddons['addons']))
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Active Add-ons') }}</h3>
                        ...
                        <div class="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                            <div>
                                ...
                                @if ($addon['activated_at'])
                                    <span class="text-xs text-gray-400 ml-2">{{ __('since :date', ['date' => $addon['activated_at']->format('M j, Y')]) }}</span>
                                @endif
                            </div>
                ...
                <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('Total add-ons') }}</span>
        ...
        @if($billingHistory->isNotEmpty())
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Billing History') }}</h3>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                <th class="text-left py-2 pr-4 font-medium text-gray-500 dark:text-gray-400">{{ __('Event') }}</th>
                                <th class="text-left py-2 pr-4 font-medium text-gray-500 dark:text-gray-400">{{ __('Date') }}</th>
                                <th class="text-right py-2 font-medium text-gray-500 dark:text-gray-400">{{ __('Amount') }}</th>
                            </tr>
                        </thead>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6" x-data="{ yearly: false }">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">{{ __('Available Plans') }}</h3>
            ...
            <div class="flex justify-center" style="margin: 30px 0">
                <div class="bg-gray-100 dark:bg-gray-700 rounded-full p-1 flex items-center gap-1">
                    ...
                    <button @click="yearly = true" :class="yearly ? 'bg-white dark:bg-gray-600 shadow text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'" class="px-4 py-2 rounded-full text-sm font-medium transition">
                        {{ __('Yearly') }} <span class="text-green-600 dark:text-green-400 font-semibold">-20%</span>
                    </button>
                </div>
            </div>
                        ...
                        <div class="text-center">
                            ...
                            <div class="mt-3">
                                <div x-show="!yearly">
                                    ...
                                    <span class="text-sm text-gray-500 dark:text-white">/{{ __('mo') }}</span>
                                </div>
                                <div x-show="yearly" x-cloak>
                                    <span class="text-3xl font-bold text-gray-900 dark:text-white">${{ number_format($yearlyMonthly) }}</span>
                                    <span class="text-sm text-gray-500 dark:text-white">/{{ __('mo') }}</span>
                                    <p class="text-xs text-green-600 dark:text-green-400 mt-1">${{ number_format($yearlyPrice) }}/{{ __('yr') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-auto pt-4">
                            ...
                            @elseif($isSubscribed)
                                ...
                                <a :href="`{{ route('billing.checkout', ['plan' => $planKey]) }}&period=${yearly ? 'yearly' : 'monthly'}`"
                                   ...
                                   onclick="return confirm('{{ $isUpgrade ? __('Switch to :plan plan? Charges will be prorated.', ['plan' => $plan['label'] ?? ucfirst($planKey)]) : __('Downgrade to :plan plan? Limits will be adjusted immediately.', ['plan' => $plan['label'] ?? ucfirst($planKey)]) }}')">
                                    {{ $isUpgrade ? __('Upgrade') : __('Switch plan') }}
                                </a>
                            @else
                                <a :href="`{{ route('billing.checkout', ['plan' => $planKey]) }}&period=${yearly ? 'yearly' : 'monthly'}`"
                                   class="block w-full rounded-lg bg-primary-600 px-4 py-2 text-center text-sm font-medium text-white hover:bg-primary-700 transition">
        ...
        @if(!empty($referral))
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Referral Program') }}</h3>
                <p class="text-sm text-gray-500 dark:text-white mb-4">
                    {{ __('Share your link and earn extra trial days. Your referral gets +7 days, you earn +15 days per registration and +15 more when they subscribe.') }}
                </p>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 dark:text-white mb-1">{{ __('Your referral link') }}</label>
                    <div class="flex gap-2">
                        ...
                        <button type="button"
                                onclick="navigator.clipboard.writeText(document.getElementById('referral-url').value); this.textContent='{{ __('Copied!') }}'; setTimeout(() => this.textContent='{{ __('Copy') }}', 2000)"
                                class="rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white hover:bg-primary-700 transition">
                            {{ __('Copy') }}
                        </button>
                    </div>
                    <p class="text-xs text-gray-400 mt-1">{{ __('Code') }}: {{ $referral['code'] }}</p>
                </div>
            ...
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-white mt-1">
                            {{ __('Update your payment method, download invoices, or cancel your subscription.') }}
                        </p>
                    </div>
    ...
    @if($this->showCancelModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" wire:click.self="closeCancelModal">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('Cancel Subscription') }}</h3>
                <p class="text-sm text-gray-500 dark:text-white mb-4">
                    {{ __('Before you go, it would help us to know why you are cancelling. Your answer is anonymous.') }}
                </p>
